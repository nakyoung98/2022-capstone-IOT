...
def check_data():
    a=('+82'+(user_id.get()[1:]))
    global firebase 
    firebase = FirebaseServer(a)

    print(a)
    window # 로그인 창을 닫고 메인 화면 생성
    newwin = Tk()
    newwin.title('복약일지')
    newwin.geometry("2000x1200")
    newwin['bg']='white'
    ###user_id.get(), 이름으로 바꿀 부분
    tk.Label(newwin, text= firebase.userInfo.name+ " 님, ",width=20, height=2, font=('맑은 고딕',35,'bold'),bg='white',
...
def check_time():
    ...
    if firebase is not None:
        ...
        tk.Label(medtime, text=medicine.morning.time.toString(),width=15, height=1,relief="groove", font=('맑은 고딕',30,'bold'),
            fg='black').place(relx=0.6,rely=0.35, anchor = "s")
        tk.Label(medtime, text=medicine.lunch.time.toString(),width=15, height=1,relief="groove", font=('맑은 고딕',30,'bold'),
            fg='black').place(relx=0.6,rely=0.45, anchor = "s")
        tk.Label(medtime, text=medicine.dinner.time.toString(),width=15, height=1,relief="groove", font=('맑은 고딕',30,'bold'),
